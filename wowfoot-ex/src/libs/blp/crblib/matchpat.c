...
bool strmatch3seq(char * str,const char c1,const char c2,const char c3)
{
...
{
	// BAD : this IsWild() is sloppy, eg. doesn't check literal makers and such
...
pattern chshMakePattern(char * spec)
{
	int len = strlen(spec);
	
	// hacky thing for chsh : 
	//	make sure we treat existing paths as literals
	//	this is to fix paths that have wild chars in the file names
	//	there will be a "\*" stuck on the existing valid name
		...
		spec[len-2] == '/') )
	{
		...
		if ( FileExists(spec) )
		{
			...
			if ( (ret = malloc(sizeof(patchar)*(len+16))) == NULL )
				{ MakePatternError = "malloc"; return(NULL);}

			//	manually build a pattern that's just all literals + a star
