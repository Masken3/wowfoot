...
void DrivePartInsert(const char *Name,char *PutIn);
...
char * DrivePart_Static(char *F)
{
DrivePartInsert(F,DrivePartRet);
return(DrivePartRet);
}

void PathPartInsert(const char *F,char *Insert)
{
char * FP;
strcpy(Insert,F);
FP = FilePart(Insert);
...
DIR	*opendir( const char * name )
{
	...
	CatPaths(tempName,"*");
	//mbstowcs(wName,tempName,1024);
	
	/*
	{
	struct _finddata_t tt;
	_findfirst(tempName,&tt);
	}
	*/
	
	d->handle = FindFirstFile(tempName,&(d->data));
	
	//d->handle = _wfindfirst(wName,&(d->data));
	//if ( d->handle == -1 )
	...
	strncpy(d->entry.a_name,d->data.cFileName,1024);
	//mbstowcs(d->entry.w_name,d->entry.a_name,1024);
	
	//wcscpy(d->entry.w_name,d->data.name);
	//wcstombs(d->entry.a_name,d->entry.w_name,1024);
...
struct dirent *readdir( DIR * d )
{
	...
	else
	{
		...
		{
			//if ( _wfindnext(d->handle,&(d->data)) == 0 )
			//{
			//	wcscpy(d->entry.w_name,d->data.name);
			//	wcstombs(d->entry.a_name,d->entry.w_name,1024);
				
			if ( FindNextFile(d->handle,&(d->data)) )
			{
				strncpy(d->entry.a_name,d->data.cFileName,1024);
			//	mbstowcs(d->entry.w_name,d->entry.a_name,1024);
			}
