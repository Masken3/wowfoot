...
void appendPath(char * onto,const char * from)
{
	if ( doQuotes )
	{
		if ( *from == '"' )
...
bool fileFunc(struct WalkInfo * WI)
{
	...
	if ( MatchPatternNoCase(WI->Name,Pattern) )
	{
		...
		{
			strcat(globWork," ");
			if ( startPath ) appendPath(globWork,WI->FullName);
			else			 appendPath(globWork,WI->Name);
		}
		else
		{
...
char *commandWorkPtr=commandWork,*commandStrPtr=commandStr;
	...
	while(*commandStrPtr)
	{
		...
		{
			*commandWorkPtr = 0;
			appendPath(commandWorkPtr,Name);
		...
		{
			*commandWorkPtr = 0;
			appendPath(commandWorkPtr,Path);
			while(*commandWorkPtr) commandWorkPtr++;
			commandStrPtr += 2;
		}
...
int spawnm_main(int argc,char *argv[])
{
			...
			|| ( argv[1][0] == '-' && tolower(argv[1][1]) == 'h' ) )
	{
		...
		errputs("   -n  no quotes around passed names [def: yes quotes]");
		errputs("");
	...
	for(i=1;i<argc;i++)
	{
		if ( !gotCommand )
		{
			if ( argv[i][0] == '-' )
			{
				...
				switch(toupper(argv[i][1]))
				{
					...
					case 'N':
						doQuotes = false;
						spawnmputs("spawnm got option: No quotes");
						break;
					case 'D':
						doDirsFlag = true;
		...
		else
		{
			...
			// must check exists first
			if ( FileExists(argv[i]) )
			{
				appendPath(commandStr,argv[i]);
			}
			else if ( patArg == NULL && IsWild(argv[i]) )
			{
				patArg = argv[i];
				if ( RunLocalFlag )
					strcat(commandStr,"@f");
...
//		system(commandStr);
		...
		PathPartInsert(filename,filepath);
	...
	else 
	{
		...
		{
			char filepath[1024];
			PathPartInsert(patArg,filepath);
		
			if ( ! WalkDir(filepath,recurseFlag,doDirsFlag,
					fileFunc,NULL,NULL) ) 
			{
