...
static HANDLE SymGetProcessHandle()
{
	if (IsNT())
		//	if (0)
...
static BOOL __stdcall My_ReadProcessMemory (HANDLE, LPCVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize, LPDWORD lpNumberOfBytesRead)
{
...
sym_engine::sym_engine (unsigned _address)
: m_address(_address), m_ok(false), m_pframe(0)
{
}
...
unsigned sym_engine::symbol(char * buf, unsigned len, unsigned * pdisplacement)
{
	...
	BYTE symBuf [ 512 ] ;
	PIMAGEHLP_SYMBOL pSym = (PIMAGEHLP_SYMBOL)&symBuf;
	memset(pSym, 0, sizeof(symBuf)) ;
	pSym->SizeOfStruct = sizeof(IMAGEHLP_SYMBOL) ;
	pSym->MaxNameLength = sizeof(symBuf) - sizeof(IMAGEHLP_SYMBOL);
	...
	r = snprintf(buf, len, "%s()", pSym->Name);
...
#ifdef __GNUC__
#define __except(filter)	catch(...){} if(0)	//not supported: never execute this block
#define __finally			catch(...){} ___finally:		//label for the "finally" block
...
static DWORD WINAPI tproc(void * pv)
{
	...
	__try
	{
		...
		if ((DWORD)-1 == SuspendThread(p->thread))
		{
			p->signal  = -1;
			__leave;
		}
		...
		__finally
		{
			VERIFY((DWORD)-1 != ResumeThread(p->thread));
		}
	}
...
bool sym_engine::stack_trace(std::ostream& os, unsigned skip, const char * fmt)
{
	...
	if (worker)
	{
		...
		if ((DWORD)-1 != ResumeThread(worker))
		{
			unsigned debug_only = 0;
			// Konstantin, 14.01.2002 17:21:32
